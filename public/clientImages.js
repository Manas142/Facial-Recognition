window.onload = function () {
    ...
    document
        .getElementById("copySortedLinkBtn")
        .addEventListener("click", () => copySortedLink(eventId));
};
...
async function startRecognition(eventId, username) {
    try {
        ...
        if (data.userImages !== null) {
            // window.location.href = `userImages.html?eventId=${eventId}&username=${username}`;
            window.open(`userImages.html?eventId=${eventId}&username=${username}`,'_blank').focus();
            // displayUserImages(data.userImages, username);
        } else {
...
function displayUserImages(userImages, username) {
...
function copySortedLink(eventId) {
    const username = prompt("Enter Your Name");
    if (!username) {
        console.error("Username is missing.");
        return;
    }
    const link = `http://localhost:3000/userImages.html?eventId=${encodeURIComponent(eventId)}&username=${encodeURIComponent(username)}`;
