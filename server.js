...
// const fs = require('fs'); // Add this import if not already present

app.delete("/events/:eventId/photos/:photoId", async (req, res) => {
  ...
  try {
    ...
    const filePath = path.join(__dirname, `public/events/${eventId}/${photo.path}`);

    // Check if the file exists
    if (fs.existsSync(filePath)) {
      await fs.promises.unlink(filePath); // Delete the file from the filesystem
    } else {
...
app.get("/userImages.html", (req, res) => {
  res.sendFile(path.join(__dirname, "public", "userImages.html"));
});
// app.get("/user-images/:eventId/:username", async (req, res) => {
//   const { eventId, username } = req.params;

//   try {
//     const event = await Event.findById(eventId);
//     if (!event || !event.userImages || !event.userImages[username]) {
//       return res.status(404).json({ success: false, message: "No images found for this user." });
//     }

//     const userImages = event.userImages[username];
//     res.json({ success: true, userImages });
//   } catch (error) {
//     console.error("Error fetching user images:", error);
//     res.status(500).json({ success: false, message: "Error fetching user images." });
//   }
// });
...
app.get("/user-images/:eventId/:username", async (req, res) => {
  ...
  console.log(`Fetching images for eventId: ${eventId}, username: ${username}`); // Log request details
  ...
  try {
    ...
    if (!event.userImages || !event.userImages.get(username)) {
      console.error(`No images found for user: ${username} in eventId: ${eventId}`);
      return res.status(404).json({ success: false, message: "No images found for this user" });
    }

    const userImages = event.userImages.get(username);
    const imagesWithPath = userImages.map((image) => {
      return path.join("events", eventId, image);
    })
    console.log(`Found ${userImages.length} images for user: ${username} in eventId: ${eventId}`);
    res.json({ success: true, userImages: imagesWithPath });
  } catch (error) {
    console.error("Error fetching user images:", error);
    res.status(500).json({ success: false, message: "Error fetching user images" });
  }
...
app.post("/generate-link", (req, res) => {
  ...
  if (!eventId || !username) {
    return res.status(400).json({ success: false, message: "Missing eventId or username." });
  }

  const cipher = crypto.createCipher('aes-256-cbc', 'your_secret_key');
  let encrypted = cipher.update(`${eventId}:${username}`, 'utf8', 'hex');
  encrypted += cipher.final('hex');

  const link = `http://localhost:3000/userImages.html?data=${encodeURIComponent(encrypted)}`;
...
app.get("/decrypt-data", (req, res) => {
  ...
  const [eventId, username] = decrypted.split(':');
  res.json({ success: true, eventId, username });
});
...
// });
app.get("/clientImages.html", (req, res) => {
  const { id: eventId, name: eventName, username } = req.query;

  if (username) {
    // If the username is present, serve the clientImages.html which will fetch and display the sorted images
    res.sendFile(path.join(__dirname, "public", "clientImages.html"));
  } else {
    // Serve the clientImages.html file
    res.sendFile(path.join(__dirname, "public", "clientImages.html"));
...
app.post("/events/:eventId/photos", async (req, res) => {
  ...
  try {
    ...
    const updatedPhotos = event.photos.filter(photo => !imageIds.includes(photo._id.toString()));
    ...
    res.status(500).json({ success: false, message: "Error deleting images. Please try again." });
  }
});
